...
provider_name = os.environ.get("PRICEWATCH_PROVIDER", "groq")
...
for idx, (row, cache_file) in enumerate(missing_rows, 1):
    ...
    print(f"[{idx}/{len(missing_rows)}] Processing {snap_id} ({store})...")
    obs = None
    err_type = None
    ...
    for attempt in range(5):
        try:
            ...
            print(f"ProviderError attempt {attempt+1}: {e}")
    ...
    if obs is not None:
        ...
        print(f"Success for {snap_id}: price_cents={obs.price_cents}")
    else:
        cache_file.write_text(json.dumps({"error": err_type or "provider_error", "latency_ms": lat}), encoding="utf-8")
        print(f"Failed for {snap_id}: error={err_type}")
